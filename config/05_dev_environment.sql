...
--   SELFHEALING_PROD and re-grants SELFHEALING_PIPELINE narrow
...
BEGIN
    ...
    GRANT CREATE TABLE ON SCHEMA SELFHEALING_DEV.GOLD
      TO ROLE SELFHEALING_PIPELINE;
    GRANT OWNERSHIP ON ALL TABLES IN SCHEMA SELFHEALING_DEV.SILVER
      TO ROLE SELFHEALING_PIPELINE COPY CURRENT GRANTS;
    GRANT OWNERSHIP ON ALL TABLES IN SCHEMA SELFHEALING_DEV.GOLD
      TO ROLE SELFHEALING_PIPELINE COPY CURRENT GRANTS;

    RETURN 'SELFHEALING_DEV refreshed — grants applied to SELFHEALING_PIPELINE';
END;
$$;
