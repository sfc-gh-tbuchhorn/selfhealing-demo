...
-- Advance pipeline_status on existing rows if re-running this script
ALTER TABLE SELFHEALING_PROD.CONFIG.SCHEMA_CHANGE_EVENTS
    ADD COLUMN IF NOT EXISTS pipeline_status   VARCHAR DEFAULT 'PENDING';
ALTER TABLE SELFHEALING_PROD.CONFIG.SCHEMA_CHANGE_EVENTS
    ADD COLUMN IF NOT EXISTS branch_name       VARCHAR;
ALTER TABLE SELFHEALING_PROD.CONFIG.SCHEMA_CHANGE_EVENTS
    ADD COLUMN IF NOT EXISTS generated_changes VARIANT;
ALTER TABLE SELFHEALING_PROD.CONFIG.SCHEMA_CHANGE_EVENTS
    ADD COLUMN IF NOT EXISTS test_output       VARCHAR;
